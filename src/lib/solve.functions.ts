...
const SYSTEM_PROMPTS: Record<Mode, string> = {
  ...
  full:
       `You are a STEM teacher. Explain step by step like in school curriculum. ${LANGUAGE_RULE} Use LaTeX for ALL math expressions in content fields, wrapped with $...$ inline or $$...$$ display. For the "formula" field: output ONLY pure LaTeX without any text or delimiters, example: "\\frac{d}{dx}[x^2\\sin(x)] = 2x\\sin(x) + x^2\\cos(x)". Return ONLY valid JSON (no markdown, no code fences) with this exact shape: {"concept": string, "steps": [{"title": string, "content": string, "formula"?: string}], "answer": string}`,
  socratic:
